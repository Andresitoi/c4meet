...
export const createRectangle = ({ x, y }) => {
  setState((state) => {
    state.shapes[nanoid()] = {
      ...
      cornerRadius: DEFAULTS.RECT.CORNERRADIUS,
      x,
      y,
    };
  });
...
export const transformArrowShape = (node, id, event) => {
  ...
  setState((state) => {
    ...
    if (shape) {
      ...
      arrowEnd.x = node.width()* scaleX;
      console.log(arrowEnd.x);

      shape.points = [0,0,arrowEnd.x,0];
      shape.width = clamp(
        node.width() * scaleX,
        LIMITS.ARROW.MIN,
        LIMITS.ARROW.MAX
